  ...
  useEffect(() => {
    ...
    if (room?.id) {
      const fetchBookedDates = async () => {
        try {
          const response = await datPhongService.getDatPhongByMaPhong(room.id);
          const roomBookings: DatPhongResponse[] = Array.isArray(response.data?.content)
            ? response.data.content
            : [response.data?.content].filter((b): b is DatPhongResponse => !!b);
          ...
          setBookedDates(Array.from(disabledDatesSet));
        } catch (error) {
          console.error("Failed to fetch booked dates:", error);
          toast.error("Không thể tải được lịch đặt phòng.");
        }
      };
      fetchBookedDates();
    }
  ...
  return (
    <div className="border p-6 rounded-2xl shadow-lg space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Số khách (tối đa {room.khach})</label>
          <select
            value={guests}
